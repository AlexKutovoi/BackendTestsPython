...
def test_login_locked_out_user(page):
    ...
    error = steps.login_page.get_error()
    assert 'locked out' in error, 'Wait message about locked user'
...
def test_logout(page):
    ...
    catalog.logout()
    assert page.url == 'https://www.saucedemo.com/', 'Wait return on login page'
